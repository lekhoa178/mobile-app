...
function GameScreen() {
  ...
  const [noticePanel, setNoticePanel] = useState(<View />);
  ...
  const validateAns = () => {
    ...
    if (questionsType[curQuestion] >= 1) {
      let content = "";
      if (vietnamese === ansStr) correctAns++;
      else content = vietnamese;

      setNoticePanel(
        <NoticePanel closeHandle={closeHandle} correct={vietnamese === ansStr}>
          {content}
        </NoticePanel>
      );
    } else {
      ...
      if (english === ansStr) correctAns++;
      else content = english;

      setNoticePanel(
        <NoticePanel closeHandle={closeHandle} correct={english === ansStr}>
          {content}
        </NoticePanel>
      );
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.headerContainer}>
        <Title
          oStyle={{ fontSize: 25, padding: 0, textTransform: "capitalize" }}
        >
          {questionTitle}
        </Title>
      ...
      <AnswerContainer selectHandle={optionSelectHandle} />

      <OptionContainer selectHandle={optionSelectHandle} />

      {confirm
        ? <View style={styles.confirmContainer}>
            <Pressable style={styles.buttonConfirm} onPress={validateAns}>
        ...
        : <View style={styles.confirmContainer}>
            <Pressable
              style={[
                ...
                { backgroundColor: "#57dc42", borderColor: "#259b13" }
              ]}
